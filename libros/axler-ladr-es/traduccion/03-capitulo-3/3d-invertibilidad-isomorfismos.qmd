...
> **[NOTA EDITORIAL · $\mathbf F^0$].** En 1.10–1.11, Axler introduce $\mathbf F^n$ para $n>0$. El teorema 3.70 también abarca $V=\{0\}$: la base vacía y la única aplicación de $\{0\}$ en $\{0\}$ dan un isomorfismo. Para que la afirmación «$V$ es isomorfo a $\mathbf F^{\dim V}$» incluya literalmente este caso, esta edición define $\mathbf F^0=\{()\}$, el espacio vectorial formado por la lista vacía (admitida en 1.8). Esta convención es propia y distinta de la ampliación de $\mathbf F^{m,n}$ indicada junto a 3.72.
...
> **[NOTA EDITORIAL · dimensión cero].** La notación $\mathbf F^{m,n}$ de 3.39 y la fórmula $\dim\mathbf F^{m,n}=mn$ de 3.40 se introducen allí para $m,n>0$. Si $V=\{0\}$ o $W=\{0\}$, el resultado 3.72 también vale: la única aplicación lineal $V\to W$ es la aplicación cero, de modo que $\dim\mathcal L(V,W)=0=(\dim V)(\dim W)$. Para leer 3.71 en esos casos ampliamos *por convención de esta edición* $\mathbf F^{m,n}$ a $m=0$ o $n=0$ como el espacio formado por la única matriz de ese tamaño, admitida en 3.29; entonces $\mathcal M$ es el isomorfismo entre dos espacios de dimensión cero. Esta ampliación no se atribuye a Axler.
...
> **[NOTA · lectura de la fórmula].** $C$ convierte coordenadas de la base $\mathcal U$ a la base $\mathcal V$: $[x]_{\mathcal V}=C[x]_{\mathcal U}$. Para obtener $[Tx]_{\mathcal U}$, se aplica primero $C$, luego $B$ y por último $C^{-1}$. Ese recorrido explica el orden $C^{-1}BC$.
